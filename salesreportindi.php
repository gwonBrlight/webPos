...
$result1 = mysqli_query($result1,"SELECT * FROM salessumarry WHERE (name='$daycusname') AND (date BETWEEN '$a' AND '$b')");
...
$result1 = mysqli_query($result1,"SELECT sum(total) FROM salessumarry WHERE (name='$daycusname') AND (date BETWEEN '$a' AND '$b')");
while($row = mysqli_fetch_array($result1))
{
    $rrr=$row['sum(total)'];
